...
void MainWindow::processRadarData(const QString &data) {
    ...
    if (parts.size() == 4) {
        float servoAngle = parts[0].toFloat();
        float measurementAngle = parts[1].toFloat();
        float distance = parts[2].toFloat();
        float height = parts[3].toFloat();

        updateDetectionPoint(servoAngle, measurementAngle, distance);
        updateHeightDisplay(height);

        // Update labels
        ui->angleLabel->setText(QString("Servo Angle: %1°, Measurement Angle: %2°").arg(servoAngle, 0, 'f', 1).arg(measurementAngle, 0, 'f', 1));
...
void MainWindow::updateDetectionPoint(float servoAngle, float measurementAngle, float distance) {
    if (distance > 0 && distance <= 200) {
        float totalAngle = servoAngle + measurementAngle;
        float radAngle = qDegreesToRadians(totalAngle);
    ...
    float radAngle = qDegreesToRadians(servoAngle);
